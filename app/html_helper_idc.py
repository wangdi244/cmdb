...
def Page(page,all_page_count):
    ...
    '''
    page_html = []
    frist_html = "<a href='/idc/%d'>首页</a>" %(1)
    page_html.append(frist_html)
    
    if page<=1:
        prev_html="<a href='#'>上一页</a>"
    else:
        prev_html ="<a href='/idc/%d'>上一页</a>" %(page-1,)
    ...
    for i in range(begin,end):
        if page == i+1:
            a_html = "<a class='selected' href='/idc/%d'>%d</a>" %(page,page)
        else:
            a_html = "<a href='/idc/%d'>%d</a>" %(i+1,i+1)
        page_html.append(a_html)
    
    next_html = "<a href='/idc/%d'>下一页</a>" %(page+1,)
    page_html.append(next_html)
    
    end_html = "<a href='/idc/%d'>尾页</a>" %(all_page_count)
